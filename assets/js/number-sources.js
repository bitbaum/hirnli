/**
 * Number Sources Database
 * Central registry for all numbers displayed in the Hirn Dashboard
 * 
 * Every number must have:
 * - Unique ID
 * - Source type (source/derived/estimated/target)
...
const NumberSources = {
  ...
  'financial_total_2025': {
    ...
    documentation: {
      ...
      drivers: ['Anzahl verkaufter Geräte', 'Durchschnittlicher Verkaufspreis', 'Dienstleistungsaufträge'],
      improvements: ['Mehr Geräte beschaffen', 'Marketing verstärken', 'Dienstleistungen erweitern'],
  ...
  'financial_self_financing_2025': {
    ...
    source: {
      type: 'derived',
      confidence: 'high',
      dependencies: ['financial_total_2025', 'financial_warenverkauf_2025', 'financial_dienstleistungen_2025']
    },
    formula: {
      type: 'custom',
      expression: '(warenverkauf + dienstleistungen) / total * 100',
      dependencies: ['financial_warenverkauf_2025', 'financial_dienstleistungen_2025', 'financial_total_2025']
    },
    validation: {
      rules: [
        { type: 'range', min: 0, max: 100 },
        { type: 'crosscheck', message: 'Sollte mit Summe der Kategorien übereinstimmen' }
      ]
    },
    documentation: {
      description: 'Anteil der Einnahmen aus eigener Wirtschaftstätigkeit (ohne Spenden). Ziel: >70%',
      drivers: ['Warenverkauf', 'Dienstleistungen', 'Gesamteinnahmen'],
      improvements: ['Warenverkauf steigern', 'Dienstleistungen ausbauen', 'Spendenabhängigkeit reduzieren'],
      target: 70,
      link: '../methodik/index.html#self-financing',
      whyItMatters: 'Eigenfinanzierung zeigt Unabhängigkeit von Spenden. >70% = nachhaltig, <40% = gefährlich abhängig.'
    }
  },
  ...
  'devices_estimated_2025': {
    ...
    documentation: {
      description: 'Geschätzt aus Warenverkauf-Einnahmen. Exakte Stückzahlen werden nicht in Kivitendo erfasst.',
      limitations: ['Zubehör mitgezählt', 'Grosse Preisvariation (CHF 50-500)', 'Durchschnittspreis ist Schätzung'],
  ...
  'co2_total_2025': {
    ...
    source: {
      ...
      assumption: '300 kg CO₂ pro Gerät (Durchschnitt)'
    },
    formula: {
      type: 'custom',
    ...
    documentation: {
      description: 'Berechnet aus geschätzter Geräteanzahl. Basiert auf wissenschaftlichen LCA-Studien.',
      limitations: ['Geräteanzahl ist Schätzung', 'Durchschnittswert, nicht gerätespezifisch'],
      improvements: ['Exakte Gerätezahlen erfassen', 'Gerätespezifische CO₂-Werte verwenden'],
  ...
  'ewaste_total_2025': {
    ...
    documentation: {
      ...
      limitations: ['Gerätegewicht variiert stark (Laptop ~2kg, Desktop ~10kg)', 'Basiert auf Geräteschätzung'],
  ...
  'erfolgsquote_40': {
    ...
    source: {
      ...
      dataSource: '03_Teilhabe_Reintegration_Tracking.csv'
    },
    formula: {
      type: 'custom',
      expression: 'COUNTIF(Reintegration_Erfolg ∈ ["Ja - Festanstellung", "Ja - Ausbildung", "Ja - Selbständigkeit"]) ÷ COUNT(Follow_up_Datum) × 100'
    },
    validation: {
      rules: [
        { type: 'range', min: 0, max: 100 },
        { type: 'warning', message: 'Zielwert - aktuell nicht systematisch getrackt' }
      ]
    },
    documentation: {
      description: 'Zielwert für erfolgreiche Wiedereingliederungen. Follow-up 6-12 Monate nach Programmabschluss. Erfolgreich = Festanstellung, Ausbildung oder Selbständigkeit.',
  ...
  'praktikanten_100': {
    ...
    source: {
      type: 'historical',
      confidence: 'low',
      description: 'Geschätzte Anzahl seit Gründung'
    },
    formula: null,
    validation: {
      rules: [
        { type: 'warning', message: 'Historische Schätzung, nicht systematisch erfasst' }
      ]
    },
    documentation: {
      description: 'Geschätzte Anzahl Praktikant:innen seit Gründung 2009. Exakte Zahl nicht systematisch erfasst.',
...
/**
 * Get number source information
 */
function getNumberSource(key) {
 ...
 */
function getNumbersByCategory(category) {
  return Object.values(NumberSources).filter(n => n.category === category);
}

/**
 * Get all numbers by dimension
 */
function getNumbersByDimension(dimension) {
  return Object.values(NumberSources).filter(n => n.dimension === dimension);
}
...
function validateNumber(key, value) {
  ...
  if (!source) return { valid: false, error: 'Source not found' };
    ...
    source.validation.rules.forEach(rule => {
      if (rule.type === 'range') {
        if (value < rule.min || value > rule.max) {
          results.valid = false;
          results.errors.push(`Value ${value} outside range [${rule.min}, ${rule.max}]`);
        }
      }
