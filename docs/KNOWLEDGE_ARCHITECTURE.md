...
**3. Foundation Data**
- **Write SSOT:** PostgreSQL `fundraising_foundations` table (self-hosted PostgreSQL)
- **Read layer:** `src/lib/db/foundations-repo.ts` — cached DB reads at runtime
  (`unstable_cache`, 1h TTL; the old generated-TS cache and its sync script are deleted)
- **Contains:** ~1,684 foundations in the read layer (full pipeline
  is ~16,500 active rows; ~241 P1-P3 actionable). Run `pnpm run audit`
  for live counts.
...
> that exists today: `pnpm run validate:foundations` (schema +
> duplicate + quality-gate via `validateFoundationQuality` SSOT —
> see `scripts/foundation-validate.ts`), `pnpm run audit` (pipeline
> funnel report), `pnpm run lint:umlauts` (Swiss German enforcement),
> and `pnpm test` (Vitest, ~1,000 tests). The proposals here remain
