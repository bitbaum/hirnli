...
export default function BudgetDetail({ inspector }: { inspector: InspectorHandle }) {
  const inspectBudgetTotal = NumberSources.budget_total_y1
    ...
    : {
        label: 'Gesamtbudget Jahr 1',
        value: formatCHF(BUDGET_SUMMARY.total),
  ...
  const inspectJaehrlich = NumberSources.budget_jaehrlich
    ...
    : {
        ...
        value: formatCHF(BUDGET_JAEHRLICH_TOTAL),
        sourceType: 'derived' as const,
        source: 'BUDGET_MODULES',
        confidence: 'Hoch',
      };
  ...
  const inspectStiftungen3Y = NumberSources.stiftungen_3y_total
    ...
    : {
        label: 'Stiftungsfinanzierung 3 Jahre',
        value: formatCHF(STIFTUNGEN_3Y_TOTAL),
        sourceType: 'derived' as const,
        ...
        <div className="mb-3 flex items-baseline justify-between">
          <h3 className="heading-card">
            ...
            <span className="ml-2 text-sm font-normal text-text-muted">
              (degressiv finanziert über 3 Jahre)
            </span>
          </h3>
          <Inspectable
            data={inspectJaehrlich}
            inspector={inspector}
            className="text-lg font-bold tabular-nums text-pillar-vision"
          >
            {formatCHF(BUDGET_JAEHRLICH_TOTAL)}
            <span className="text-sm font-normal text-text-muted">/Jahr</span>
          </Inspectable>
        </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Davon {formatCHF(BUDGET_SUMMARY.eigenleistung)} Eigenleistung* (
              {BUDGET_SUMMARY.selfFinancingPct}%)
            </p>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium text-pillar-vision">Stiftungs-Förderbedarf</div>
            <Inspectable
              data={inspectStiftungen3Y}
              inspector={inspector}
              className="text-xl font-bold tabular-nums text-pillar-vision"
            >
