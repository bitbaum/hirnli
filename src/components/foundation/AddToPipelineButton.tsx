...
export default function AddToPipelineButton({
  ...
  if (state === 'conflict') {
    return (
      <div className="space-y-2">
        <p className="text-sm text-warning-text">{foundationName} ist bereits in der Pipeline.</p>
  ...
  if (state === 'success') {
    return (
      <div className="space-y-3">
              ...
              <span
                className={
                  i === 0
                    ...
                    : 'rounded bg-surface-raised px-2 py-0.5 text-text-muted'
                }
              >
                {i === 0 ? `✓ ${step}` : step}
              </span>
        ...
        <Button href={`/fundraising/stiftungen/${foundationId}/gesuch`} fullWidth>
          Gesuch schreiben →
        </Button>
        <Button href="/fundraising/applications" variant="secondary" fullWidth>
          Pipeline ansehen
  ...
  return (
    <div className="space-y-1">
      <Button onClick={handleClick} disabled={state === 'loading'} fullWidth>
        {state === 'loading' ? 'Wird hinzugefügt...' : 'Gesuch starten'}
      </Button>
      {state === 'error' && error && <p className="text-sm text-danger-text">{error}</p>}
    </div>
  );
}
