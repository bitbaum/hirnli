  ...
  return (
    <div className={`rounded-lg border-l-4 ${tier.color.border} ${tier.color.bg} p-3`}>
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className={`heading-detail ${tier.color.text}`}>{tier.shortName}</span>
...
export default function GesuchProcessSection({ showFuture = false }: GesuchProcessSectionProps) {
  const model = showFuture ? CASCADE_MODELS.future : CASCADE_MODELS.current;

  return (
    <section>
      <h2 className="mb-6 heading-page">{CASCADE_SUMMARY.headline}</h2>
      <p className="mb-6 text-base leading-relaxed text-text-secondary">{CASCADE_SUMMARY.body}</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {model.tiers.map((tier, i) => (
            <div key={tier.id} className="flex items-center gap-2">
              ...
              {i < model.tiers.length - 1 && <span className="text-text-muted">&rarr;</span>}
            </div>
          ))}
        </div>
      </Card>
      ...
      <Callout color="primary" className="px-5">
        <p className="text-sm font-medium text-text leading-relaxed">{CASCADE_SUMMARY.insight}</p>
      </Callout>
    </section>
  );
}
