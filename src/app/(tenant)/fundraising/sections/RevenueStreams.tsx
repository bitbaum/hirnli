...
import { REVENUE_STREAMS, REVENUE_CURRENT_TOTAL, REVENUE_YEAR3_TOTAL } from '../data';
...
export default function RevenueStreamsSection({ inspector }: { inspector: InspectorHandle }) {
  const inspectRevenueCurrent = NumberSources.revenue_current
    ...
    : {
        label: 'Einnahmen aktuell',
        value: formatCHF(REVENUE_CURRENT_TOTAL),
  ...
  const inspectRevenueYear3 = NumberSources.revenue_year3
    ...
    : {
        label: 'Einnahmen-Prognose Jahr 3',
        value: formatCHF(REVENUE_YEAR3_TOTAL),
        sourceType: 'estimated' as const,
        source: 'Geschäftsplan',
        confidence: 'Mittel',
      };
    ...
    <section className="mb-8">
      ...
      <div className="mb-6 rounded-xl border border-success/20 bg-success/10 p-4">
        <p className="text-sm text-text-primary mb-2">
          <strong>Aktuell:</strong> {formatCHF(REVENUE_CURRENT_TOTAL)}/Jahr (B2B Services +
          Geräteverkauf + Integration + Spenden)
        </p>
        <p className="heading-item text-success mb-1">
          <strong>Ziel Jahr 3:</strong> {formatCHF(REVENUE_YEAR3_TOTAL)}/Jahr (Prognose) — das wäre
          +
          {Math.round(
            ((REVENUE_YEAR3_TOTAL - REVENUE_CURRENT_TOTAL) / REVENUE_CURRENT_TOTAL) * 100,
          )}
          % Wachstum
        </p>
        ...
        {REVENUE_STREAMS.map((stream) => {
          ...
          return (
            <Card key={stream.source} className={`${isNew ? 'border-l-4 border-l-success' : ''}`}>
              ...
              {growth !== null && (
                <div className="mt-2 text-sm font-medium text-success">Wachstum: +{growth}%</div>
              )}
            </Card>
          );
        })}
