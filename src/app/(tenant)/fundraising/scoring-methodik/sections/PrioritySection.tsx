    ...
    <section className="mb-10">
      ...
      <Card>
        <p className="mb-4 text-sm text-text-secondary">
          Die Priorität kombiniert Fit und Bereitschaft zu einer handlungsorientierten Empfehlung.
          Fit wirkt als <strong>Multiplikator</strong> (Gate), nicht als Summand — eine Stiftung
          ohne thematischen Fit hat immer Priorität 0, unabhängig von der Datenlage.
        </p>
        ...
        <div className="mb-4 rounded-lg bg-surface-raised p-4">
          ...
          <div className="space-y-1 font-mono text-sm text-text-secondary">
            <p>
              basis = fit_normiert × ({PRIORITY_FORMULA.baseFitFloor} +{' '}
              {PRIORITY_FORMULA.readinessScale} × bereitschaft / 100)
            </p>
            <p>priorität = min(100, basis × min(anwendbare_abzüge) + förder_bonus)</p>
          </div>
          <p className="mt-2 text-sm text-text-muted">
            fit_normiert = fitScore / 10 (0-1). Bei Fit 10 + Bereitschaft 100 → Priorität 100. Bei
            Fit 10 + Bereitschaft 0 → Priorität {PRIORITY_FORMULA.baseFitFloor}. Bei Fit 0 → immer
            0.
          </p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border-default text-left">
                  <th scope="col" className="pb-2 pr-4 font-medium text-text-muted">
                    Bedingung
                  </th>
                  <th scope="col" className="pb-2 text-right font-medium text-text-muted">
                    Multiplikator
                  </th>
                  <th scope="col" className="pb-2 text-right font-medium text-text-muted">
                    Effekt
        ...
        <div className="mb-4">
          ...
          <p className="mb-2 text-sm text-text-secondary">
            Wenn der Förderbereich der Stiftung mit unserem Zielbetrag (CHF{' '}
            {formatNumber(PRIORITY_FORMULA.grantMatch.targetMin)}–
            {formatNumber(PRIORITY_FORMULA.grantMatch.targetMax)}) übereinstimmt:
          </p>
          <div className="space-y-1 text-sm text-text-secondary">
            <p>
              Perfekte Überlappung:{' '}
              <span className="font-medium text-primary">
                +{PRIORITY_FORMULA.grantMatch.perfectBonus}
              </span>
            </p>
            <p>
              Nahe dran (max ≥ Hälfte unseres Minimums):{' '}
              <span className="font-medium text-primary">
                +{PRIORITY_FORMULA.grantMatch.closeBonus}
              </span>
            </p>
            <p>
              Betrag bekannt aber klein:{' '}
              <span className="font-medium text-primary">
                +{PRIORITY_FORMULA.grantMatch.smallBonus}
              </span>
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {PRIORITY_FORMULA.display.map((d) => {
              ...
              return (
                <div
                  key={d.level}
                  className="flex items-center justify-between rounded-lg border border-border-default/50 p-3"
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="w-10 text-right text-sm font-bold tabular-nums text-text-primary">
                      {priorityCounts[d.level]}
                    </span>
                  </div>
                </div>
              );
            ...
            <div className="flex items-center justify-between rounded-lg border border-border-default/50 p-3">
              <div className="flex items-center gap-3">
                <span
                  className={`rounded px-1.5 py-0.5 text-xs font-bold ${PRIORITY_CONFIG[4].color}`}
                >
                  {PRIORITY_CONFIG[4].label}
                </span>
                <span className="text-sm text-text-secondary">
                  {PRIORITY_CONFIG[4].description}
                </span>
              </div>
              <div className="flex items-center gap-3">
          ...
          <p className="mt-2 text-sm text-text-muted">
            Durchschnittlicher Prioritäts-Score: {priorityAvg}/100. Manuell überschriebene
            Prioritäten sind mit «manuell» gekennzeichnet.
          </p>
        </div>
      </Card>
    </section>
