...
export function useFoundationFilters(foundations: Foundation[]) {
  ...
  const filters: FoundationFilters = useMemo(
    () => ({
      ...
      schwerpunkt: (() => {
        ...
        return sp && isSchwerpunktId(sp) ? sp : null;
      })(),
  ...
  const setSchwerpunkt = useCallback(
    (sp: SchwerpunktId | null) => {
      updateParams({ sp: sp });
    },
    [updateParams],
  );
  ...
  const togglePriorityLevel = useCallback(
    (level: number) => {
  ...
  const { filtered, scoreMap } = useMemo(() => {
    ...
    if (isSearching) {
      ...
      const result =
        sort === 'priority' ? structuralFiltered : sortFoundations(structuralFiltered, sort);
