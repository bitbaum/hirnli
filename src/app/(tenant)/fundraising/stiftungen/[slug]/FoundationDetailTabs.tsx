...
export default function FoundationDetailTabs({
  foundation: f,
  fitNarrative,
  themeAlignments,
  approachSteps,
    ...
    <>
      {showTrustWarning && (
        <Callout color="warning" className="mb-4 text-sm font-medium">
          Automatisch eingestuft — Fit-Score und Priorität basieren auf KI-Analyse, nicht auf
          manueller Recherche. Vor einer Bewerbung bitte über die Recherche-Links verifizieren.
        </Callout>
      )}
      {banner && (
                          ...
                          <table className="mt-2 w-full text-sm">
                            <thead>
                              <tr className="border-b border-border-default">
                                <th
                                  scope="col"
                                  className="py-2 text-left text-sm font-semibold text-text-muted"
                                >
                                  Eingabe bis
                                </th>
                                <th
                  ...
                  <div className="space-y-4 text-sm">
                    ...
                    {f.criteria && (
                      <div>
                        ...
                        {f.criteria.nature && (
                          <p className="mt-1 text-text-secondary">{f.criteria.nature}</p>
                        )}
                        {f.criteria.education && (
                          <p className="mt-1 text-text-secondary">{f.criteria.education}</p>
                        )}
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex gap-2">
                          {f.sdgs.map((sdg) => (
                            <span
                              key={sdg}
                              className="rounded bg-surface-raised px-2 py-1 text-xs font-medium"
                            >
                              SDG {sdg}
                            </span>
                          ))}
                      ...
                      <div>
                        ...
                        <div className="mt-2 space-y-1">
                          {f.boardMembers.map((m) => (
                            <div
                              key={m.name}
                              className="flex items-baseline justify-between border-b border-border-default py-1"
                            >
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {f.pastGrantees.map((g) => (
                            <span key={g} className="rounded bg-surface-raised px-2 py-1 text-xs">
                              {g}
                            </span>
                          ))}
                        </div>
                          ...
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap gap-1">
                              {f.memberships.map((m) => (
                                <span
                                  key={m}
                                  className="rounded bg-accent-muted px-2 py-0.5 text-xs font-medium text-primary"
                                >
                                  {m}
                                </span>
                              ))}
