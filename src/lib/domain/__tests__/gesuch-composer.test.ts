...
describe('composeGesuch', () => {
  ...
  it('returns ready=false when no themes map', () => {
    ...
    const result = composeGesuch(STORY, f);
    expect(result.ready).toBe(false);
  ...
  it('falls back to zuerich badge when zuerich is the only theme', () => {
    ...
    const result = composeGesuch(STORY, makeFoundation({ themes: ['zuerich'] }));
    expect(result.ready).toBe(true);
    if (result.ready) {
      expect(result.themes.all.length).toBeGreaterThan(0);
  ...
  it('accepts schwerpunktId override', () => {
    const withDefault = composeGesuch(STORY, makeFoundation());
    const withSchwerpunkt = composeGesuch(STORY, makeFoundation(), 'nachhaltigkeit');
...
describe('composeAnschreibenText', () => {
  ...
  it('puts the GIVEN organisation in the subject, not an imported one', () => {
    ...
    expect(composeAnschreibenText(STORY, f).subject).toContain(TENANT.name);
    ...
    const other = composeAnschreibenText(makeStory(makeTenant({ name: 'Andere Organisation' })), f);
  ...
  it('uses schwerpunkt theme label in subject when schwerpunktId provided', () => {
    ...
    const withDefault = composeAnschreibenText(STORY, f);
    const withSchwerpunkt = composeAnschreibenText(STORY, f, 'soziale-integration');
